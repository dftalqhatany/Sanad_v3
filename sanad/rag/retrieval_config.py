"""Read the RAG's retrieval configuration *statically* (AST), without importing it.

Importing rag/retriever.py connects to Qdrant and loads the embedding model, so the constants are
parsed from the source instead. This keeps rag/retriever.py the single source of truth for the
Qdrant URL, collection, embedding model, top-k and fusion weight.
"""
...
from models.regulatory import RetrievalConfig
...
def _parse(path: Path) -> ast.Module:
    try:
        ...
        raise RetrievalConfigError(f"Cannot read the retrieval configuration from {path}: {exc}") from exc


def read_retrieval_config(path: Path) -> RetrievalConfig:
    tree = _parse(path)
    ...
    if missing:
        raise RetrievalConfigError(f"{path} no longer defines literal constants: {', '.join(missing)}")
...
def read_answer_model(path: Path) -> str | None:
    """Model name passed to chat.completions.create(...) in rag/backend.py (informational)."""
    try:
        tree = _parse(path)
