"""Programmatic interface (adapter) over the RAG in this package (rag/retriever.py, rag/backend.py).
...
What it does:
  1. lazily imports `rag.backend` (which builds the HybridRetriever over the `saudi_labor_law`
     Qdrant collection),
  2. calls those functions unchanged,
...
# The field these fill (RegulatoryEvidenceResult.legacy_entry_point) keeps its name for API
# compatibility; the values now name the in-tree modules the call actually goes through.
LEGACY_RETRIEVE_ENTRY_POINT = "rag/retriever.py::HybridRetriever.retrieve (via rag.backend.get_retriever)"
LEGACY_ANSWER_ENTRY_POINT = "rag/backend.py::answer_policy_question"
...
class RegulatoryRAGAdapter:
    def __init__(
        ...
        loader: RagLoader | None = None,
        clock: Callable[[], float] = time.perf_counter,
    ) -> None:
        self.settings = settings or SanadSettings.from_env()
        self._loader = loader or RagLoader(self.settings.rag_dir, self.settings.backend_module)
        self._clock = clock
        self._config_warnings: list[str] = []
        try:
            self._retrieval_config: RetrievalConfig | None = read_retrieval_config(
        ...
        self._answer_model = read_answer_model(self.settings.backend_module_path)
    ...
    def health(self, *, check_vector_db: bool = True, qdrant_url: str | None = None) -> RagHealth:
        """Read-only readiness check. Never imports the backend module and never writes to Qdrant."""
        ...
        if not files_present:
            errors.append(make_error(RagErrorCode.LEGACY_RAG_NOT_FOUND, "health_check",
                                     f"RAG files not found under {settings.rag_dir}"))
        ...
        return RagHealth(
            status=status,
            legacy_rag_dir=str(settings.rag_dir),   # RagHealth field name kept for API compatibility
