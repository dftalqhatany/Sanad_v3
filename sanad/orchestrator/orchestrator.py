...
RAG modules, an LLM or a parser directly where an existing abstraction exists.
"""
...
from config import AnalysisSettings, DocumentProcessingSettings, SanadSettings
...
from models.regulatory import RegulatoryAnswerResult, RegulatoryEvidenceResult, RegulatoryQuery
from orchestrator.errors import OrchestratorErrorCode, orchestrator_error
from orchestrator.intake import DocumentIntake, IntakeItem
...
class RegulatoryQuestionSource(Protocol):
    """The adapter (the regulatory RAG agent's interface), never rag.backend or rag.retriever themselves."""
...
class SanadOrchestrator:
    ...
    def from_settings(cls, settings: SanadSettings | None = None, analysis_settings: AnalysisSettings | None = None,
        ...
        if evidence_source is None:
            from rag.adapter import RegulatoryRAGAdapter  # imported lazily: nothing else loads the RAG
