...
---------------------
OPENAI_API_KEY         Key passed to the existing RAG's answer generation (never hard-coded, never logged).
SANAD_LEGACY_RAG_DIR   Location of the existing RAG project. Defaults to ../hr_assistant next to this project.

The Qdrant URL, collection name, embedding model, top-k and fusion weight are NOT configured
here: they are read from the existing hr_assistant/hybird_search.py so the legacy RAG stays the
single source of truth (see sanad.rag.legacy_config).
"""
...
PROJECT_ROOT = Path(__file__).resolve().parent.parent  # .../llm-zoomcamp/sanad
REPO_ROOT = PROJECT_ROOT.parent  # .../llm-zoomcamp
DEFAULT_LEGACY_RAG_DIR = REPO_ROOT / "hr_assistant"


@dataclass(frozen=True)
class SanadSettings:
    legacy_rag_dir: Path = DEFAULT_LEGACY_RAG_DIR
    openai_api_key: str | None = field(default=None, repr=False)

    # Names of the existing RAG files (the retrieval module name is misspelled in the original project).
    legacy_backend_module: str = "chatbot_backend"
    legacy_backend_file: str = "chatbot_backend.py"
    legacy_retrieval_file: str = "hybird_search.py"
    ...
    # Provenance of the regulatory source, as documented in hr_assistant/README.md.
    ...
    def __post_init__(self) -> None:
        object.__setattr__(self, "legacy_rag_dir", Path(self.legacy_rag_dir).expanduser().resolve())

    @classmethod
    def from_env(cls, env: Mapping[str, str] | None = None) -> "SanadSettings":
        env = os.environ if env is None else env
        return cls(
            legacy_rag_dir=Path(env.get("SANAD_LEGACY_RAG_DIR") or DEFAULT_LEGACY_RAG_DIR),
            openai_api_key=(env.get("OPENAI_API_KEY") or None),
        )

    @property
    def backend_module_path(self) -> Path:
        return self.legacy_rag_dir / self.legacy_backend_file

    @property
    def retrieval_module_path(self) -> Path:
        return self.legacy_rag_dir / self.legacy_retrieval_file

    @property
    def knowledge_base_path(self) -> Path:
        return self.legacy_rag_dir / self.knowledge_base_relpath

    @property
    def source_document_path(self) -> Path:
        return self.legacy_rag_dir / self.source_document_relpath
...
class ApiSettings:
    ...
    SANAD_API_HOST           Host for `python -m sanad.api` (default 127.0.0.1).
    SANAD_API_PORT           Port for `python -m sanad.api` (default 8000).
    """
