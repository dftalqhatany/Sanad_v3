"""The adapter drives the REAL, unmodified hr_assistant RAG code and knowledge base.
...
# --------------------------------------------------------------------------- retrieval
def test_retrieve_evidence_returns_exactly_what_the_legacy_hybrid_retriever_returns(adapter, infra, knowledge_base, legacy_dir):
    ...
    assert Path(legacy_backend().__file__).resolve().parent == legacy_dir.resolve()
    ...
    assert result.legacy_entry_point.startswith("hr_assistant/hybird_search.py::HybridRetriever.retrieve")
...
def test_ask_uses_existing_answer_policy_question_and_returns_its_evidence(adapter, infra, knowledge_base):
    ...
    assert result.status is ResultStatus.SUCCESS, result.errors
    # legacy post-processing (strip + highlight_articles) came from chatbot_backend, not from Sanad
    ...
    assert result.legacy_entry_point == "hr_assistant/chatbot_backend.py::answer_policy_question"
...
# --------------------------------------------------------------------------- failures
def test_ask_without_api_key_is_explicit_and_never_touches_the_rag(legacy_dir, infra, monkeypatch):
    monkeypatch.delenv("OPENAI_API_KEY", raising=False)
    adapter = RegulatoryRAGAdapter(SanadSettings(legacy_rag_dir=legacy_dir, openai_api_key=None))
...
def test_qdrant_unreachable_while_loading_existing_rag_is_explicit_and_recoverable(adapter, infra, real_qdrant_connection_error):
    infra.vector_store_init_error = real_qdrant_connection_error  # raised inside hybird_search.py at import
    ...
    assert "chatbot_backend" not in sys.modules and "hybird_search" not in sys.modules
...
def test_missing_knowledge_base_is_explicit(tmp_path, legacy_dir, infra):
    for name in ("chatbot_backend.py", "hybird_search.py"):
        shutil.copy(legacy_dir / name, tmp_path / name)  # copies in a temp dir; originals untouched
    adapter = RegulatoryRAGAdapter(SanadSettings(legacy_rag_dir=tmp_path, openai_api_key=TEST_KEY))
    result = adapter.retrieve_evidence("What is the maximum probation period?")
    assert result.status is ResultStatus.ERROR
    assert (result.errors[0].code, result.errors[0].stage) == ("knowledge_base_missing", "initialization")
