...
TOOLS = sorted((PROJECT_ROOT / "sanad" / "tools").rglob("*.py"))
PROVIDER = PROJECT_ROOT / "sanad" / "agents" / "salary_web.py"
SALARY_FILES = [*TOOLS, PROVIDER, PROJECT_ROOT / "sanad" / "agents" / "salary.py"]
PACKAGE_FILES = sorted((PROJECT_ROOT / "sanad").rglob("*.py")) + sorted((PROJECT_ROOT / "frontend").rglob("*.py"))
...
def test_the_tools_know_nothing_about_agents_the_rag_or_the_api():
    forbidden = ("sanad.agents", "sanad.rag", "sanad.orchestrator", "sanad.api", "chatbot_backend", "hybird_search",
...
def test_the_provider_does_no_http_itself():
    ...
    assert "sanad.tools.web_search" in modules  # it goes through the client abstraction
...
def test_keys_and_limits_come_from_settings():
    config = (PROJECT_ROOT / "sanad" / "config.py").read_text(encoding="utf-8")
...
def test_the_documented_sources_and_their_ranking_are_declared():
    from sanad.config import SALARY_ALLOWED_DOMAINS, SALARY_SEED_SOURCES
    from sanad.tools.salary_sources import profile_for
...
def test_the_comparison_reads_the_benchmark_rather_than_recomputing_it():
    dimensions = (PROJECT_ROOT / "sanad" / "agents" / "comparison_dimensions.py").read_text(encoding="utf-8")
